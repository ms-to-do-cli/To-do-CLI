...
async function incomplete(this: I): Promise<void> {
    ...
    const { listName }: { listName: string } = await Interactive.prompt([{
        ...
        name: 'listName',
        suggestOnly: true,
        message: 'From which TaskList do you want to incomplete a Task',
    ...
    const tasks = (await taskList.getTasks()).filter(t => t.status !== 'notStarted');

    if (tasks.length === 0)
        throw new Error('No Tasks found that are completed yet');
    ...
    const { taskName }: { taskName: string } = await Interactive.prompt([{
        type: 'autocomplete',
        name: 'taskName',
        message: 'Choose the Task you want to incomplete',
        suggestOnly: true,
    ...
    this.log(`Task ${task.title} incompleted`);
}
